...
- source-proven corrections:
  - `ஆண்டுகளே` → `ஆண்டுகளேனோ`;
  - `வைத்து படைத்துக்` → `வைத்துப் படைத்துக்`;
  - false hyphen after `திருப்பியிருந்தனர்` → source full stop;
  - `மெய்மறந்து` → `மெய்யும் மறந்து`;
- apparent source shape at `இல்லை.` explicitly checked as historical **`லை`** identity; Unicode `இல்லை.` remains correct;
- `புரு வலுவிலே` retained as the source wording;
