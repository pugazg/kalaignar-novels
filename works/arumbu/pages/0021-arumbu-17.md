---
scan_page: 21
printed_page: 17
work: "arumbu"
section: "அரும்பு"
page_type: "body"
status: "verified"
language: "ta"
source_filename: "TVA_BOK_0064361_அரும்பு.pdf"
transcription_method: "direct visual transcription from source scan; T1 complete; independent historical-glyph T2 re-read complete; final source-fidelity T3 complete"
---

அதை எடுத்து மார்போடு அணைத்துக்கொண்டான். படுக்கைக்கு வந்தான் படத்தோடு! அம்மா படத்தின் பக்கத்திலே படுத்துக் கொண்டான். “அந்தக் கட்டிலிலே பாப்பாவும் அதன் அம்மாவும்! இங்கே, இந்தக் கட்டிலிலே அம்மாவும் நானும், அய்யோ!” இப்படி ஒரு வித்தியாச விஷத்தை அந்த அரும்பின் உள்ளத்திலே தெளிப்பதற்குக் காரணமாயிருந்தவள் கோமதிதான்!

இல்லை! இல்லை! நோய் வடிவான அவள் பிள்ளைதான்!

கோமதியின் அருகே கிடந்த பாப்பா அழுதது; கோமதி நன்றாகத் தூங்கிக்கொண்டிருந்தாள். பலநாள் கண் விழித்த களைப்பு அவளுக்கு! குமார் எழுந்து பார்த்தான். சன்னல்கள் எல்லாம் மூடிக் கிடந்தன.

‘பாப்பாவுக்குக் காற்றில்லை; அதனால் அழுகிறது!’ - இது அவன் எண்ணம். சன்னல் கதவுகளை நன்றாகத் திறந்துவிட்டான். சிலுசிலுவெனப் பனிக்காற்று வீச ஆரம்பித்தது. பாப்பாவும் அந்தக் குளிர்காற்று பட்டதும் மெளனமாகிவிட்டது. குமாரும், தான் நினைத்தது சரிதான் என்ற மகிழ்ச்சியில் பாப்பாவைத் தட்டிக் கொடுத்துவிட்டுத் தன் படுக்கைக்குப் போனான்.

குழந்தைக்குப் பனி கூடாது என டாக்டர் எச்சரித்தது அவனுக்கு என்ன தெரியும்? தூங்கிவிட்டான். அனைவரும் ஆழ்ந்து தூங்கினர். விடியற்காலை! கோமதி விழித்துப் பார்த்தாள், சன்னல் திறந்திருந்தது.

“அய்யோ! யார் சன்னலைத் திறந்தது?” என்று கத்தினாள். கோகுல் திடுக்கிட்டெழுந்தான்.

“நான்தானம்மா, பாப்பாவுக்குக் காற்று வேணும்னு திறந்தேன்!” என்று கூறியபடி எழுந்தோடி வந்தான் குமார்.

கோமதி, குழந்தையைத் தொட்டுப் பார்த்தாள். குழந்தை விறைத்துக் கிடந்தது. அது இறந்து அதிக நேரமாகிறது! முதலில் ஒரு முறை அழுததே, அப்போதே அது போய்விட்டது! ஆனால் பழி... குமாரின் தலையிலே விழுந்தது! அங்கிருந்த ஒரு கூஜாவை எடுத்துக் குமாரின் தலையிலே ஓங்கி வீசி எறிந்தாள் கோமதி! “அய்யோ! அம்மா!” என அலறினான் குமார்! கூஜா குறி தவறி - அவன் படுக்கையில் இருந்த அம்மா படத்தின்மீது விழுந்து, தூள் தூளாகப் போயிற்று. குமார் படத்தை எடுத்து முகத்தில் ஒற்றிக்கொண்டு அழுதான். உடைந்த கண்ணாடித் துண்டுகள் அவன் அழகு முகமெங்கும் கீறி, இரத்தம் கசிந்தது! கோகுலுக்கு எதை நினைத்து அழுவது என்று புரியவில்லை.

குமார் இரண்டு மூன்று நாட்களாகச் சரியாகச் சாப்பிடவே இல்லை. பாப்பாவைக் குழியிலே வைத்துப் புதைத்ததற்கு அவனல்லவா காரணமாம்? அய்யோ, அப்பா பாப்பாவை நினைத்து

## T1 direct-transcription notes

- printed page **17** is directly visible;
- scan 20 ends with `அவனது அம்மா படம்!`; scan 21 begins `அதை எடுத்து...`, preserving the physical continuation;
- scan 21 ends physically at `அப்பா பாப்பாவை நினைத்து`, continuing onto scan 22;
- lower `க. க—2` is non-body/footer material and is not transcribed into the narrative body.

## T2 independent historical-glyph notes

- complete page independently re-read at enlarged/native source resolution;
- explicitly checked the full known set `ணா / ணை / ணொ / ணோ / லை / ளை / றா / றொ / றோ / னா / னை / னொ / னோ`;
- representative source-confirmed identities include `அணைத்துக்கொண்டான்` (`ணை`), `கண்ணாடித்` (`ணா`), `இல்லை` (`லை`), `பிள்ளைதான்` (`ளை`) and `நன்றாகத்` (`றா`);
- families without a direct occurrence on this page were checked as absent, not inferred;
- T2 character-identity corrections: **0**;
- unresolved historical glyphs: **0**.

## T3 final source-fidelity notes

- complete page independently compared line-by-line with direct source pixels;
- corrected T1 punctuation `குமார் எழுந்து பார்த்தான்:` → **`குமார் எழுந்து பார்த்தான்.`**;
- corrected T1 punctuation `சன்னல்கள் எல்லாம் மூடிக் கிடந்தன:` → **`சன்னல்கள் எல்லாம் மூடிக் கிடந்தன.`**;
- printed page **17**, body-page identity, 20→21 and 21→22 continuity, and footer separation were confirmed;
- T2 character identities remain confirmed;
- unresolved source readings: **0**.

<!-- source physical scan: 21; printed page: 17; T1 PASS / T2 PASS / T3 PASS / VERIFIED -->
