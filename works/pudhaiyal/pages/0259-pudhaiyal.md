---
scan_page: 259
printed_page: 255
work: "pudhaiyal"
section: "29"
page_type: "body"
status: "verified"
language: "ta"
source_filename: "TVA_BOK_0064097_புதையல்.pdf"
transcription_method: "user-supplied Iteration 24 continuation reconciled directly against high-resolution native split part 006 page 14 image; old-Tamil-typeform pre-correction check applied"
---

“வயிறே காலியாக இருக்கும்போது, எப்படி அய்யா கக்குவது? என்னமோ எனக்கு உண்மையெல்லாம் அஜீர்ணமாகிக் கிடக்கிற மாதிரி பேசுறீங்களே!”

“டே, பேச்சாளி! கதையெல்லாம் அளக்காதே! தங்கம் புதைக்கப்பட்டிருக்கிற இடத்தை மறைக்காம சொல்லி விடு!”

“அட கர்மமே; தங்கமாவது! மண்ணுங் கட்டியாவது! எனக்கு ஒன்றுமே விளங்க வில்லையே! அட கடவுளே! இது என்னப்பா சோதனை — என்னப்பா வேதனை!”

“இந்த பாரு—பயப்படாதே! நயமாவே கேட்கிறேன், சொல்லு! நம்ப ரெண்டு பேருமே கூட்டா சேர்ந்து புதையலை எடுப்போம்; யாருக்கும் தெரியாம அதை பாதுகாத்து பணக்காரனாவோம்! தம்பீ, துக்காராம்; உண்மை உன்னிடமிருக்கிறது; உழைப்பு என்னிடமிருக்கிறது! உண்மையையும், உழைப்பையும் ஒன்றாய்ப் பிசைந்து ஒற்றுமையாக காரியம் செய்தால், இந்த உலகத்திலேயே இரண்டே பணக்காரர்கள் தான் இருப்பார்கள். ஒன்று நீ! அடுத்தது நான்! என்ன சொல்லுகிறாய்?”

“அய்யா — நான் உண்மையைச் சொல்லட்டுமா?”

“சொல்லு தம்பீ சொல்லு! நல்ல பிள்ளையாண்டானல்லவா நீ — சொல்லுப்பா சொல்லு!”

“உண்மையாவே ஒரு உண்மையும் எனக்குத் தெரியாது பராபரமே!”

“பார்த்தாயா வாலாட்டுகிறாய்! ஏய் திமிர் பிடித்தவனே — கையிலே என்ன இருக்கிறது பார்!”

வெள்ளியம்பலம் ஒரு பாளை சீவும் அரிவாளை எடுத்து உயர்த்திக் காட்டினார். அதைக் கண்ட துக்காராம் விழி பிதுங்கும் அளவுக்கு பயந்து போய் நின்றான்.

## Source / boundary notes

- Chapter 29 continues.
- Native high-resolution inspection establishes source `மண்ணுங் கட்டியாவது!`; baseline `மண்ணாங் கட்டியாவது!` is not substituted.
- Source preserves separated `விளங்க வில்லையே!` and source colloquialisms such as `மறைக்காம`, `தெரியாம`, and `உண்மையாவே`.
- High-resolution old-typeform inspection confirms `ஒன்றாய்ப் பிசைந்து`; no terminal/vowel-sign uncertainty remains.
- This closes the remaining Iteration-24 text represented on printed pages 252–255. Scan 260 / printed 256 begins the supplied Iteration-25 baseline.

<!-- மூல ஸ்கேன் பக்கம்: 259; அச்சுப் பக்கம்: 255 -->