---
scan_page: 7
printed_page: 6
work: "balipeedam-nokki"
section: "பலிபீடம் நோக்கி"
page_type: "body"
status: "verified"
language: "ta"
source_filename: "TVA_BOK_0065570_பலிபீடம்_நோக்கி.pdf"
transcription_method: "direct visual comparison with source scan"
---

ஆறுகளைப் பாருங்கள். நீங்களும் பலிபீடம் நோக்குங்கள்.

வீழ்ந்துபோக அல்ல!... வாழ்ந்து வா! பலிபீடம் நோக்கிகளே !... இதோ மாடலவறையன் மரபிலே ஒருவர்!...

வேதியர் வெங்கண்ணா! திரைப் படமாக உருப்பெற்றிருக்கிறார்... படக்காட்சி ஆரம்பமாகிறது பாருங்கள். படம் உங்களுக்கு ஒரு பாடம் தரட்டும்.

திரையில் ஒரு எரிமலை குமுறுகிறது. ‘குபீர் குபீர்’ என்று நெருப்புக் குழம்பை வாரி வீசுகிறது. அந்த நெருப்பு அபிஷேகத்திற்கு ஆளாகி இடிந்து சரியும் ஆலயங்களின் கோபுரங்கள், மடாலய மதிற்சுவர்கள், பண்ணைக்காரரின் களஞ்சியங்கள் அனந்தம்... அனந்தம்! பயங்கரப் பின்னணி சங்கீதம் ‘கட கடா... குடு குடு..’ என்ற அகோர இரைச்சல்.

எரிமலையைச் சுவராகக்கொண்டு பல எழுத்துக்கள் உருள்கின்றன.

★

## Scan observations

- This page completes the `பலிபீடம் நோக்கி` textual unit and transitions rhetorically toward the following cinematic-style work.
- A printed black star ornament appears below the final line and is preserved above.
- Reverse-side bleed-through is visible across the lower half of the page; it has not been treated as current-page text.
- Light age/stain marks appear near the page edges.

<!-- மூல ஸ்கேன் பக்கம்: 7; அச்சுப் பக்கம்: 6 -->
