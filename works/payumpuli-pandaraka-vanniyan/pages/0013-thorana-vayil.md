---
scan_page: 13
part: 1
part_page: 13
printed_page: 2
work: "payumpuli-pandaraka-vanniyan"
section: "தோரண வாயில்"
page_type: "body"
status: "verified"
visual_fidelity: "verified"
language: "ta"
source_filename: "TVA_BOK_0065744_பாயும்புலி_பண்டாரக_வன்னியன்_part_001_pages_1-30.pdf"
transcription_method: "direct source-pixel transcription; Part001 Pass 1 complete; formal Part001 Pass 2A reviewed; formal Part001 Pass 2B lexical/historical-glyph reread reviewed; formal Part001 Pass 3 visual/structural review completed"
---

# தோரண வாயில்

## Source transcription

தோரண வாயில்

வீரம், விவேகம், தியாகம் இவை எந்தத் திசையிலிருந்து ஒளி உமிழ்ந்தாலும் அதனைப் போற்றிப் புகழ்க் காவியம்படைக்க வேண்டுமென்ற ஆர்வமுடையவன் நான்!

அதிலும் அந்த வீரமும், விவேகமும், தியாகமும் தமிழ்க் குலத்தின் சொத்துக்களாக எங்கேயிருப்பினும், அவை கால வெள்ளத்தின் கொடுமையால் கவனிப்பாரற்றுப் போய்விடக் கூடாது என்ற கவலை எனக்கு மிக உண்டல்லவா?

அதன் விளைவாகத்தானே தமிழனின் பழங்கால வரலாற்று அடிப்படையில் “ரோமாபுரிப் பாண்டியன்” என்ற பெரு நூலையும், அதற்கடுத்து பிற்காலத் தமிழ்மானச் செம்மல் “தென்பாண்டிச் சிங்கம்” என்ற பெரு நூலையும், அதனைத் தொடர்ந்து புறநானூறக் கதைகளை ஆதாரமாகக் கொண்டு, போர்ப்புலிகளாம் “பொன்னர்-சங்கர்” எனும் வரலாற்றுக் கற்பனைக் கதையினையும் என்னால் தமிழர்க்குத் தர முடிந்தது!

அந்த ஆர்வத்தின் அடுத்த முயற்சியாகத்தான் பாயும் புலி பண்டாரக வன்னியனின் தீரமிகு தியாக சரித்திரக் கதையைத் தீட்டிட முற்படுகிறேன்.

ஆறு ஆண்டுகளுக்கு முன்பு (1982-ல்) இலங்கையில் வவுனியா மாவட்ட வளர்ச்சிக் குழுவினர், வவுனியாவில் கற்சிலை மடுவு என்ற இடத்தில் பண்டாரக வன்னியனுக்கு சிலையெடுக்கும்

## Pass 1 notes

- complete printed text on this physical scan was transcribed directly from the attached source pixels;
- source spelling, punctuation, paragraph structure and page-boundary fragments are preserved;
- historical Tamil glyph identity was checked at Pass-1 level and encoded as the source-supported Unicode character identity without spelling modernization;
- no OCR, web transcription, later edition or grammar-based reconstruction was used as authority;
- visible printed page number: 2;
- the decorative தோரண வாயில் heading is printed text and is retained;
- the final phrase continues onto scan 14 and is intentionally not completed here;
- formal Part001 Pass 2A is complete for this scan; Pass 2B / Pass 3 remain pending; status stays `needs-review`.

## Formal Part001 Pass 2A review

- direct word-by-word, punctuation, paragraph, displayed-lineation and page-boundary comparison completed against the attached source pixels;
- source-text corrections: **0**;
- Pass 2A result: **REVIEWED / PASS AFTER ANY CORRECTIONS ABOVE**;
- page remains `needs-review` pending whole-Part Pass 2B / Pass 3 and later final status synchronization.

## Formal Part001 Pass 2B lexical / historical-glyph reread

- independently reread the complete printed source text from enlarged source pixels rather than merely confirming Pass 2A;
- checked the full known reform-sensitive set where present: `ணா / ணை / ணொ / ணோ / லை / ளை / றா / றொ / றோ / னா / னை / னொ / னோ`;
- representative source occurrences checked on this scan: `கவலை` (`லை`), `கற்பனைக்` (`னை`), `சிலையெடுக்கும்` (`லை`);
- source-text / lexical / historical-glyph corrections: **0**;
- unresolved historical-glyph identities: **0**;
- no global replacement or spelling modernization was used;
- Pass 2B result: **REVIEWED / PASS**;
- page remains `needs-review` pending whole-Part Pass 2B closure, Pass 3, Part audit and final status synchronization.

## Formal Part001 Pass 3 visual / structural review

- chapter opening uses a large ornate `தோரண வாயில்` display integrated with decorative scrollwork and a circular medallion motif;
- body prose begins below the decorative heading inside the page frame;
- printed page number `2` is centered in the dark circular footer marker;
- the final clause remains open into scan14.
- body-vs-non-body separation, displayed hierarchy, page furniture and continuation state were checked directly against the whole-page source image;
- canonical source text required **0 textual corrections** in Pass 3;
- unresolved visual/structural questions: **0**;
- Pass 3 result for this scan: **REVIEWED / PASS**;
- page remains `needs-review` until whole-Part Pass 3, Part audit and final metadata/status synchronization are complete.

<!-- மூல மொத்த ஸ்கேன் பக்கம்: 13; பகுதி: 001; பகுதி உள்ளூர் பக்கம்: 13; PASS 2A + PASS 2B + PASS 3 REVIEWED / needs-review -->
