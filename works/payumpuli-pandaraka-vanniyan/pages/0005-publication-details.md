---
scan_page: 5
part: 1
part_page: 5
printed_page: null
work: "payumpuli-pandaraka-vanniyan"
section: "front-matter"
page_type: "publication-page"
status: "needs-review"
visual_fidelity: "needs-review"
language: "ta"
source_filename: "TVA_BOK_0065744_பாயும்புலி_பண்டாரக_வன்னியன்_part_001_pages_1-30.pdf"
transcription_method: "direct source-pixel transcription; Part001 Pass 1 complete; formal Part001 Pass 2A reviewed; formal Part001 Pass 2B lexical/historical-glyph reread reviewed; formal Part001 Pass 3 visual/structural review completed"
---

## Source transcription

நூல் அளவை

தலைப்பு        : பாயும்புலி பண்டாரக வன்னியன்  
ஆசிரியர்       : கலைஞர் மு. கருணாநிதி  
பொருள்         : வரலாற்றுக் காவியம்  
வெளியீட்டகம்   : ராக்போர்ட் பப்ளிகேஷன்ஸ்(பி)லிட்.,  
                  சென்னை 600 032  
வெளியீடு       : 5  
பதிப்பு         : ஒன்று  
தாள்           : மாப் லித்தோ  
அளவு           : 14x21½ சி. எம்  
படிகள்         : 3000  
காலம்          : மே-1991  
விலை           : ரூ. 125/-(ரூபாய் நூற்றி இருபத்தைந்து மட்டும்)  
எழுத்து        : 12 புள்ளி  
ஓவியம்         : ஜெயராஜ்  
முகப்பு         : ஓவியக் கவிஞர் அமுதோன்.

கணிப்பொறி  
ஒளி அச்சு      : ராக்போர்ட் பிரஸ்-சென்னை-32  
அச்சிட்டோர்     : ராக்போர்ட் பிரஸ்-சென்னை-32

## Non-body / copy-specific marks

- an Anna Centenary Library stamp substantially overlaps the middle of the page;
- handwritten accession/classification/invoice-style markings are later copy-specific marks;
- those marks are kept separate from the printed bibliographic block;
- no printed page number is visible.

## Pass 2A correction

The earlier canonical record carried a source-legibility hold and did not transcribe the bibliographic block. Enlarged direct source review now supports the complete printed block above. The placeholder-only record is therefore replaced with page-aligned source text; no metadata was imported from an external source.

## Formal Part001 Pass 2A review

- compared directly against the attached Part001 source pixels;
- words, punctuation, displayed lineation, paragraph/page structure and separation of non-body marks were checked;
- page remains `needs-review`; formal Pass 2B / Pass 3 are still pending.

## Formal Part001 Pass 2B lexical / historical-glyph reread

- independently reread the complete printed source text from enlarged source pixels rather than merely confirming Pass 2A;
- checked the full known reform-sensitive set where present: `ணா / ணை / ணொ / ணோ / லை / ளை / றா / றொ / றோ / னா / னை / னொ / னோ`;
- representative source occurrences checked on this scan: `கலைஞர்` (`லை`), `கருணாநிதி` (`ணா`), `சென்னை` (`னை`);
- source-text / lexical / historical-glyph corrections: **0**;
- unresolved historical-glyph identities: **0**;
- no global replacement or spelling modernization was used;
- Pass 2B result: **REVIEWED / PASS**;
- page remains `needs-review` pending whole-Part Pass 2B closure, Pass 3, Part audit and final status synchronization.

## Formal Part001 Pass 3 visual / structural review

- `நூல் அளவை` is a centered display heading;
- bibliographic information is arranged as a two-column label/value block rather than ordinary prose;
- a large Anna Centenary Library stamp and handwritten accession/classification/invoice-style marks overlap the middle of the printed block; they remain copy-specific non-body matter;
- faint reverse show-through is visible behind the page; no printed page number is visible.
- body-vs-non-body separation, displayed hierarchy, page furniture and continuation state were checked directly against the whole-page source image;
- canonical source text required **0 textual corrections** in Pass 3;
- unresolved visual/structural questions: **0**;
- Pass 3 result for this scan: **REVIEWED / PASS**;
- page remains `needs-review` until whole-Part Pass 3, Part audit and final metadata/status synchronization are complete.

<!-- மூல மொத்த ஸ்கேன் பக்கம்: 5; பகுதி: 001; பகுதி உள்ளூர் பக்கம்: 5; அச்சுப் பக்கம்: —; PASS 2A + PASS 2B + PASS 3 REVIEWED / needs-review -->
